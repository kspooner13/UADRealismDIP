...
namespace TweaksAndFixes
{
    public class UiM
    {
        ...
        private static bool _peaceWindowCameraHooked = false;

        /// <summary>Get world position for a province so we can move the camera. Uses the province's port if it has one.</summary>
        private static Vector3 GetWorldPositionForProvince(Province province)
        {
            if (province == null) return G.cam != null ? G.cam.transform.position : Vector3.zero;
            foreach (PortElement port in CampaignMap.PortsDb.Ports)
            {
                if (port.CurrentProvince == province)
                    return port.WorldCoord;
            }
            return G.cam != null ? G.cam.transform.position : Vector3.zero;
        }

        private static bool _peaceWindowHierarchyLogged = false;

        /// <summary>When the peace/war reparation window is open, hook province list so that selecting a province moves the camera over it.</summary>
        private static void TryHookPeaceWindowCameraOnSelect(GameObject warReparationWindow)
        {
            // Only attempt once per window open so we don't spam logs every frame when list isn't found
            _peaceWindowCameraHooked = true;

            if (warReparationWindow == null || G.cam == null || CampaignController.Instance == null)
                return;

            GameObject content = FindPeaceWindowListContent(warReparationWindow);
            if (content == null)
            {
                if (!_peaceWindowHierarchyLogged)
                {
                    _peaceWindowHierarchyLogged = true;
                    Melon<TweaksAndFixes>.Logger.Msg("[Peace window] Could not find list Content. Hierarchy (first 80 lines):");
                    string hierarchy = ModUtils.DumpHierarchy(warReparationWindow);
                    string[] lines = hierarchy.Split('\n');
                    for (int i = 0; i < lines.Length && i < 80; i++)
                        Melon<TweaksAndFixes>.Logger.Msg(lines[i]);
                    if (lines.Length > 80)
                        Melon<TweaksAndFixes>.Logger.Msg($"[Peace window] ... and {lines.Length - 80} more");
                }
                return;
            }
            _peaceWindowHierarchyLogged = false;

            var children = content.GetChildren();
            if (children == null || children.Count == 0)
            {
                Melon<TweaksAndFixes>.Logger.Msg("[Peace window] List Content has no children");
                return;
            }
            var provinces = CampaignMap.Instance?.Provinces?.Provinces;
            if (provinces == null)
                return;
            ...
            foreach (GameObject child in children)
            {
                ...
                GameObject provinceLabel = child.GetChild("Province", true);
                if (provinceLabel != null)
                {
                    TMP_Text tmp = provinceLabel.GetComponent<TMP_Text>();
                    if (tmp != null) provinceName = tmp.text?.Trim();
                    if (string.IsNullOrEmpty(provinceName))
                    {
                        var textComp = provinceLabel.GetComponent<Text>();
                        if (textComp != null) provinceName = textComp.text?.Trim();
                    }
                }
                if (string.IsNullOrEmpty(provinceName))
                {
                ...
                if (string.IsNullOrEmpty(provinceName))
                {
                    var textComp = child.GetComponentInChildren<Text>(true);
                    if (textComp != null) provinceName = textComp.text?.Trim();
                }
                ...
                {
                    if (p != null && p.Name == provinceName) { match = p; break; }
                }
                if (match == null) continue;
                Province prov = match;
                btn.onClick.AddListener(new System.Action(() =>
                {
                    Vector3 pos = GetWorldPositionForProvince(prov);
                    if (pos != G.cam.transform.position)
                        G.cam.LookAtPointEx(pos, true, false, -1);
                }));
                hooked++;
            }
            if (hooked > 0)
                Melon<TweaksAndFixes>.Logger.Msg($"[Peace window] Hooked {hooked} province rows (click a province to move camera)");
        }

        /// <summary>Find the list content inside the war reparation window. Tries multiple possible hierarchies.</summary>
        private static GameObject FindPeaceWindowListContent(GameObject warReparationWindow)
        {
            GameObject window = warReparationWindow.GetChild("Window", true);
            GameObject content = null;

            if (window != null)
            {
                // Try ScrollRect (no space) like constructor UI
                content = window.GetChild("ScrollRect", true)?.GetChild("Viewport", true)?.GetChild("Content", true);
                if (content == null)
                    content = window.GetChild("Scroll View", true)?.GetChild("Viewport", true)?.GetChild("Content", true);
                if (content == null)
                    content = window.GetChild("Content", true);
                // Try ScrollRect component and use its content
                if (content == null)
                {
                    var scrollRect = window.GetComponentInChildren<ScrollRect>(true);
                    if (scrollRect != null && scrollRect.content != null)
                        content = scrollRect.content.gameObject;
                }
            }
            if (content == null)
                content = warReparationWindow.GetChild("Content", true);
            if (content == null)
            {
                var scrollRect = warReparationWindow.GetComponentInChildren<ScrollRect>(true);
                if (scrollRect != null && scrollRect.content != null)
                    content = scrollRect.content.gameObject;
            }
            return content;
            ...
            ModifyUi(basePopupsRoot).SetOnUpdate(new System.Action<GameObject>((GameObject ui) => {
                ...
                foreach (GameObject child in regularPopups)
                {
                    ...
                    if (child.active)
                    {
                        ...
                        hasPopups = true;
                        if (child.name == "WarReparationWindowUI")
                        {
                            peaceWindowActive = true;
                            if (!_peaceWindowCameraHooked)
                                TryHookPeaceWindowCameraOnSelect(child);
                        }
                    }
                }
                if (!peaceWindowActive)
                    _peaceWindowCameraHooked = false;
        ...
        public static void ApplyDockyardModifications()
        {
            ...
            ModifyUi(ModUtils.GetChildAtPath("Global/Ui/UiMain/Constructor")).SetOnUpdate(new System.Action<GameObject>((GameObject ui) => {
                ...
                ConstructorSceneUI.OnUpdate(ui);
                ConstructorArmorProtectionPanel.EnsurePanel(ui);
            }));

            ConstructorSceneUI.Create();
        }
