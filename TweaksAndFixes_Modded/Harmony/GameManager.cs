...
namespace TweaksAndFixes
{
    ...
    internal class Patch_GameManager
    {
        ...
        internal static void Postfix_ToConstructor(bool newShip, Ship viewShip, bool allowEdit, IEnumerable<Ship> allowEditMany, ShipType shipTypeNew, bool needCleanup, Player newPlayer)
        {
            ...
            // Capture blocked penetration from battle when opening constructor for this ship (refit/view after battle)
            if (viewShip != null)
                LastBattleBlockedPenetrationData.TryCaptureFromShip(viewShip);
            else
                LastBattleBlockedPenetrationData.Clear();
        }
